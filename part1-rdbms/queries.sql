...
SELECT s.sales_rep_id,s.sales_rep_name,COUNT(DISTINCT o.customer_id) AS unique_customers FROM sales_reps s
JOIN orders o ON s.sales_rep_id = o.sales_rep_id
GROUP BY s.sales_rep_id, s.sales_rep_name,
ORDER BY unique_customers DESC;

-- Q4: Find all orders where the total value exceeds 10,000, sorted by value descending
SELECT o.order_id,p.product_id,p.product_name,SUM(p.unit_price * oi.quantity) AS total_value FROM orders o
JOIN order_items oi ON o.order_id = oi.order_id
JOIN products p ON oi.product_id = p.product_id
